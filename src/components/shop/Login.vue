            ...
            <el-form :model="registerForm" ref="registerForm"  :rules="register_rules"   label-width="80px">
              ...
              <el-form-item label="图片">
                <el-upload
                  ...
                  name="file"
                  list-type="picture">
                  <img width="50px":src="registerForm.imgurl" />
        ...
        methods: {
          loginUser() {
            this.$refs['login_rules'].validate((valid) => {
              debugger;
              if (valid) {
                ajax.post('http://127.0.0.1:8080/api/user/loginUser', qs.stringify(this.login_param)).then(res => {
                  debugger
            ...
            saveregisterUser(){
             this.$refs['registerForm'].validate((valid) => {
              if (valid) {
                ajax.post('http://127.0.0.1:8080/api/user/insertUser', qs.stringify(this.registerForm)).then(res=>{
                  if (res.data.code == 200) {
                    alert('submit!');
           ...
           /*,async valiUserName(name){
            let res=await this.$axios.get('http://127.0.0.1:8080/api/user/selectName?name='+name);
            debugger
          ...
          handleAvatarSuccess(response, file) {
            //打断点 看怎么取返回值
            this.registerForm.imgrl=response.filePath;
          },
